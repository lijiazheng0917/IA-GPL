...
class GNN(torch.nn.Module):
    ...
    def forward(self, *argv):
        if len(argv) == 7:
            x, edge_index, edge_attr, prompt, training, batch, graph_emb = argv[0], argv[1], argv[2], argv[3], argv[4], argv[5], argv[6]
        elif len(argv) == 4:
        ...
        if len(argv) == 7:
            if prompt is not None:
                x, c_loss = prompt.add(x, batch, graph_emb, training=training)
        if len(argv) == 4:
            if prompt is not None:
                x = prompt.add(x)
        ...
        if len(argv) == 7:
            return node_representation, c_loss
        else:
...
class GNN_graphpred(torch.nn.Module):
    ...
    def forward(self, *argv):
        if len(argv) == 6:
            x, edge_index, edge_attr, batch, prompt, training = argv[0], argv[1], argv[2], argv[3], argv[4], argv[5]
        elif len(argv) == 4:
        ...
        if len(argv) == 6:
            node_representation, c_loss = self.gnn(x, edge_index, edge_attr, prompt, training, batch, node_representation)
        else:
            node_representation = self.gnn(x, edge_index, edge_attr)
